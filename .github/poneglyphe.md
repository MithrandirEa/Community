...
> **Date de synthèse** : 16 avril 2026  
...
8. [Évolutions récentes](#8-évolutions-récentes)
...
│   │   ├── admin.py            # Blueprint admin : /admin/* (session, photothèque, codes, packs de noms)
│   │   ├── officiel.py         # Blueprint officiel : /officiel (feed + rédaction + sender_name)
│   │   ├── population.py       # Blueprint population : /population (feed + rédaction + sender_name)
...
| `admin` | `/admin` | Gestion de session (démarrage/arrêt/reset), photothèque, codes d'accès, packs de noms |
| `officiel` | `/officiel` | Feed et rédaction de messages officiels (dont programmés, dont sender_name) |
| `population` | `/population` | Feed et rédaction de messages citoyens (dont sender_name) |
...
Session ─────────────────────┐
  ...
  ended_at                    │  Datetime réelle d'arrêt (NULL si toujours active)
  is_active                   │
  name_pack_id (FK → NamePack)│  Pack de noms fictifs associé (optionnel)
                              │
                        ┌─────┘
Message ────────────────┤ session_id (FK)
  ...
  is_published          │
  sender_name           │  Nom fictif du personnage (NULL si anonyme)
                        │
...
Photo ───────────────────────── (indépendant de la session)
  ...
  original_name                Nom original du fichier (affiché dans l'UI)
  uploaded_at
...
NamePack ────────────────────── (indépendant de la session)
  id (PK)
  name                         Nom du pack (unique)

NameEntry ──────────────────── (appartient à un NamePack)
  id (PK)
  pack_id (FK → NamePack)
  role                         'officiel' | 'population'
  label                        Nom fictif du personnage
```
...
Cellule de crise → POST /officiel
  ...
  ↳ Vérification sender_name : doit appartenir aux noms 'officiel' du NamePack actif
...
### 4.5bis Gestion des packs de noms fictifs

```
Animateur → GET /admin/namepacks
  ↳ Liste tous les NamePack avec leurs NameEntry
Animateur → POST /admin/namepacks  (NamePackForm)
  ↳ Crée un NamePack (nom unique)
Animateur → POST /admin/namepacks/<pack_id>/entries  (NameEntryForm)
  ↳ Ajoute un NameEntry(role, label) au pack
Animateur → POST /admin/namepacks/<pack_id>/delete
  ↳ Supprime le pack et ses entrées (cascade)
Animateur → POST /admin/namepacks/<pack_id>/entries/<entry_id>/delete
  ↳ Supprime un nom individuel du pack

Association à une session : l'animateur choisit un pack au démarrage (/admin/session/start)
  ↳ Session.name_pack_id = pack choisi (ou NULL si pas d'identités)
  ↳ Les interfaces officiel/population proposent les noms du pack dans un sélecteur
```
...
### Duplication des noms dans un pack (TODO #10)
La contrainte d'unicité sur `(pack_id, role, label)` n'est pas encore appliquée côté base de données ni formulaire — un même nom peut être ajouté deux fois dans un pack. Un commentaire `TODO [#10]` est présent dans `admin.py`.
...
| **NamePack** | Pack de noms fictifs créé par l'animateur, regroupant des identités pour la Cellule de crise et la Population. |
| **NameEntry** | Entrée d'un NamePack : association entre un rôle ('officiel' ou 'population') et un nom fictif affiché. |
| **sender_name** | Nom fictif optionnel associé à un message, sélectionné parmi les entrées du NamePack actif. |
| **ended_at** | Datetime réelle d'arrêt d'une session, renseignée lors d'un stop ou d'un reset. |

---

## 8. Évolutions récentes

### Depuis la synthèse du 15 avril 2026

| Composant | Évolution |
|---|---|
| `models.py` | Nouveaux modèles `NamePack` et `NameEntry` |
| `models.py` | `Session` : ajout `name_pack_id` (FK) et `ended_at` |
| `models.py` | `Message` : ajout `sender_name` |
| `models.py` | `Photo` : ajout `original_name` |
| `admin.py` | Nouvelles routes `/admin/namepacks` (CRUD pack + entrées) |
| `admin.py` | Nouveaux formulaires `NamePackForm` et `NameEntryForm` |
| `officiel.py` | Support `sender_name` dans `OfficielMessageForm` avec validation |
| `population.py` | Support `sender_name` dans `PopulationMessageForm` avec validation |
| `templates/admin/` | Nouveau template `namepacks.html` |

### TODO ouverts

| Référence | Description |
|---|---|
| TODO [#10] | Empêcher la duplication d'un nom dans un même pack (`name_entry` : contrainte unique sur `pack_id + role + label`) |
| TODO [#13] | Ajouter un avatar (`avatar_filename`) sur `NameEntry`, uploadé par l'admin et affiché dans le feed |
